...
왜 중요한가:
- **서버리스/Edge Runtime과 호환**: Vercel Functions·Next.js middleware는 요청마다 새 인스턴스가 뜨고 persistent TCP를 유지할 수 없음 → 일반 Redis 클라이언트로 접근 불가. HTTP면 가능.
...
1. **M11 `middleware.ts`의 PIN 조회**: Next.js middleware는 Edge Runtime이라 TCP 기반 Redis 클라이언트 사용 불가 → HTTP 기반이어야 함.
...
2. **middleware에서 매 요청 PIN 조회** → 1ms vs 수십 ms 차이가 카카오톡 인앱 브라우저 같은 느린 환경에서 체감
