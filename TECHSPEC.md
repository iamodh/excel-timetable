...
| 폰트 색상 반영 | 시트 셀의 폰트 색상을 웹에 그대로 적용 (공휴일 등은 관리자가 빨간 폰트로 타이핑하여 구분) |
...
| 카테고리 합계 대시보드 | 회차×카테고리 매트릭스 표 + 카테고리 클릭 시 시간표 그리드에 강조 표시 (목표 시수 대비 배치 시수 검증용, 데스크톱 전용) |
...
- 시간표 변경 알림 — 카카오 오픈채팅방 봇/알림톡(비즈메시지)/PWA 푸시 등 옵션 조사 후 매니저와 방향 확정. 구두 공지를 **보완**하는 수단으로 위치
- 여러 기수/프로그램 지원 (멀티 시트)
- 관리자 파싱 옵션 설정 UI — 시작 행/열, 자동 병합 보정, 색 tolerance, 회차 슬라이싱 토글을 KV에 저장 → `revalidateTag("timetable")`로 즉시 반영. 시트 구조 변동 시 코드 수정 없이 대응
- 구분 열 규칙 위반 검증 — 회차 블록 사이 구분 열(G, N, ...)에 값이 있거나 폭 ≥ 2 병합이 걸치면 서버에서 감지해 관리자 "시간표 최신화" UI에 에러 메시지 노출 (P0 테스트: 값 위반/병합 위반 감지)
- 시트 사용 영역 초과 경고 — 매니저가 회차/주차를 늘려 현재 fetch 범위(A1:AI100)를 넘으면 학생 화면에서 조용히 잘림. 마지막 행/열에 데이터 있으면 관리자에게 경고
- 관리자 카테고리 매트릭스 UI 손질 — 선택 행 강조 강화(더 진한 배경 + 좌측 강조 바), 회차 탭 선택 상태 시인성 점검
- 카테고리 시수 / 카테고리명 정합성 점검 — 회차별 합계 vs 실제 시트 셀 색상 카운트 직접 대조(병합 누락 자동 보정 셀, tolerance로 합쳐진 색 포함), 시트 범례명과 `categoryTargets.ts` `NO_TARGET_CATEGORIES` 공백/오타 일치 검증
...
| textColor | string | 셀 폰트 색상 (예: "#ff0000"), 시트에서 그대로 가져옴. 미지정 시 "#000000" |
...
### 7.9 관리자 카테고리 합계 대시보드

매니저가 카테고리별 목표 시수(16h 누적) 대비 실제 배치 시수를 검증하는 화면. 부족한 카테고리를 빠르게 찾고, 시트의 어디에 배치되어 있는지 시각적으로 확인하는 흐름을 지원한다.

**라우트:** `/admin/categories` — 관리자 PIN 보호, 데스크톱 전용
...
- 5회차 누적 기준 16h 고정. 새 카테고리가 시트에 추가되어도 자동으로 16h 적용.
- 매니저가 16h 외 다른 목표를 원하는 카테고리가 생기면 그때 시트 파싱 또는 KV 방식으로 확장한다 (현재는 도입하지 않음 — YAGNI).

**레이아웃**

상단: 카테고리 매트릭스 표
- 컬럼: 카테고리명 / 1회 / 2회 / 3회 / 4회 / 5회 / 합계 / 목표
- 행 정렬: 부족분 큰 순(합계 < 목표) → 구분선 → 목표 없는 카테고리(`null`)
- 회차 컬럼은 배경 살짝 회색 처리, 0인 셀은 흐리게(`text-gray-300`) 표시
- 행 클릭 = 단일 카테고리 선택 (다중 선택 미지원). 다시 클릭하면 해제.

하단: 시간표 그리드 (학생용 컴포넌트 재사용)
- `highlightCategory?: string` prop 추가
- prop이 있으면 비매칭 셀을 회색 처리(테두리만 남김), 매칭 셀은 원색 유지
- 회차 탭 전환 시에도 강조 상태 유지 (상위 페이지의 상태로 관리)

**시간 집계**

각 슬롯의 `bgColor`를 카테고리 색상과 매칭하여 해당 카테고리의 배치 시간(rowSpan 누적)을 합산한다. `isMergedContinuation: true`인 셀은 중복 카운트 방지를 위해 제외한다.
