...
- [ ] 회차 선택 버튼 스크롤 초기 위치 — 가장 오른쪽(최신 회차)에서 시작하도록 초기 스크롤 위치 조정
- [ ] 다크모드 글자 색상 오류 해결 — 다크모드에서 셀/헤더 글자가 배경과 동화되는 케이스 점검 및 수정
- [ ] favicon 교체 — create-next-app 기본 아이콘에서 프로젝트 아이콘으로 교체 (`app/favicon.ico`)
- [ ] 프로젝트 메타데이터 수정 — `app/layout.tsx`의 `metadata.title`/`description`을 프로젝트명에 맞게 변경, `<html lang>`도 `ko`로 변경
