...
**Unit Tests (P0)**
- [x] 다중 블록 파싱: 2개 이상 블록이 빈 열(G, N, ...)을 경계로 독립된 회차로 분리 (구분 열 인식 포함) → 01d9aaa
- [ ] 블록 헤더 파싱: 각 블록에서 `programName`, `period`, `location`, `totalHours` 개별 추출
- [ ] 미래 회차 필터: 오늘 기준 `period` 시작일이 미래인 블록은 결과에서 제외
