...
- [ ] 헤더에 프로그램명, 기간, 장소, 이수시간 표시
- [ ] 요일(화~월) × 시간대(09:00~17:00) 그리드 렌더링
- [ ] 각 셀에 수업명 + 부가정보 표시
- [ ] 셀 배경색이 시트와 동일하게 적용
- [ ] 2시간 연속 수업이 병합된 셀로 표시 (rowSpan)
...
**목표:** 1~5주차 클라이언트 탭 전환 + 현재 주차 자동 선택

> 📖 TECHSPEC 섹션 7.4, 7.6 참조

**Manual Tests**

- [ ] 탭 클릭 시 페이지 새로고침 없이 주차 전환
- [ ] 오늘 날짜에 해당하는 주차가 기본 선택됨
- [ ] 교육 기간 외 접속 시 1주차 표시

**완료 조건:** 주차 탭 클릭으로 즉시 전환되고, 초기 로드 시 현재 주차 자동 선택

- Commits:

---

### Milestone 7: 공휴일/휴무 표시
...
- [ ] 배포된 URL에서 시간표 정상 표시
- [ ] Google Sheets API 연동 정상 동작 (서비스 계정 환경변수)
...
### Milestone 11: 최종 QA & 배포

**목표:** 에러 처리 UI 적용 + 전체 테스트 검증 + 최종 배포
...
**완료 조건:** 에러 상황에서 사용자 메시지 정상 표시, 전체 기능 최종 확인 후 배포 완료
