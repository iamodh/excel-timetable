...
- [ ] 슬롯 → 장소/강사 조회 연결: 본 그리드 Slot에 장소/강사(없으면 null) 부착
- [ ] 바텀시트 UI — 셀 탭 시 하단에서 슬라이드업, 수업명/장소/강사 표시. 시트가 떠 있는 채로 다른 셀 탭하면 내용 교체, 바깥/닫기로 해제 (모바일·데스크톱 공통)
- [ ] 탭 가능 힌트 — 장소/강사 정보가 있는 셀에 표시(예: 📍 아이콘)
- [ ] 예외 처리 — 정보 없는 슬롯(5회차 미입력·자율수업 등)은 시트 미표시 또는 "정보 없음"
