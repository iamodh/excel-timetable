...
- [ ] 보조 테이블을 블록별 5열로 재작성: `날짜 │ 13:00 │ 14:00 │ 15:00 │ 16:00`
...
**Unit Tests**
- [ ] 보조 테이블 파싱: (날짜 × 시작 시간) → 장소/강사 텍스트 조회 테이블 생성
- [ ] 본 그리드 슬롯 매칭: 날짜 + 슬롯 시작 시간으로 조회, 없으면 null

**Implementation**
- [ ] `lib/` 보조 테이블 파서 — 헤더 행의 시작 시간을 읽어 열 매칭 (열 위치 하드코딩 금지)
- [ ] 슬롯 클릭 시 장소/강사 표시 UI (학생 시간표 그리드)
