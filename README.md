...
- 수업별 장소·강사 안내 추가 (셀을 누르면 하단에서 장소/강사 표시)
- 오늘 진행도 위젯(토더기) 추가 — 전 과정 이수 시간과 다음 수업을 한눈에
- 지난 주차는 시간표 아래로 정렬하고 구분선으로 분리
- 회차 노출을 "이번 회차 + 다음 회차"로 정리하고, 첫 화면을 오늘 회차에서 시작
- 모달이 닫힐 때 자연스러운 애니메이션 추가
- 전역 폰트를 Pretendard로 교체

### v1.1

- 시간표 보기 편의 개선 (최신 회차 자동 노출, 카테고리 색상표 상단 고정, 시간 열 폭 축소)
- 다크모드에서 글자가 잘 안 보이던 문제 수정
- 브라우저 탭 이름·아이콘을 청도지 시간표 전용으로 변경
- 시간표 로딩 중에도 공지와 가이드 메뉴는 즉시 표시되도록 변경
